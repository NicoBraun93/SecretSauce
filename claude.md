...
- **[LaunchdManager.swift](file:///Users/nico/Workspace/SecretSauce/native/Sources/SecretSauce/Services/LaunchdManager.swift):** Lists launch agents from `~/Library/LaunchAgents`, reads their runtime status from `launchctl list`, edits the `EnvironmentVariables` dict in their plists, and runs load/unload/start/stop.
  - **Session vs. persistent — the distinction the whole tab is built around:** `load`/`unload` only touch the launchd domain running right now. The plist stays in `~/Library/LaunchAgents`, which launchd re-reads at every login, so an unloaded agent is back after a reboot. `setAutostart(enabled:)` is the persistent half: `launchctl enable|disable gui/<uid>/<label>` writes launchd's override database (`/var/db/com.apple.xpc.launchd/disabled.<uid>.plist`), which survives reboots, paired with a best-effort `bootstrap`/`bootout` so the change also takes effect immediately. The `gui/<uid>` domain is the user's own, so none of this needs root. Persistent state is read back via `launchctl print-disabled gui/<uid>` (parse both `=> disabled` and `=> true`).
  - **`RunAtLoad` is read-only on purpose:** it is surfaced as the "Starts" card (At Login / On Trigger / Never) but never written. Turning Autostart on for an agent that has no `RunAtLoad` must *not* inject the key — that would silently convert an on-demand agent into a login-start one. Autostart therefore only ever touches the override database, never the user's plist.
  - **Per-agent resource cost:** one `ps -A -o pid=,ppid=,rss=,%cpu=,etime=` call builds a `ProcessSnapshot` with parent links, and `subtree(of:)` sums RSS/CPU over the job's whole process tree — a wrapper script's pid is what launchd tracks, but the real cost sits in its children. `%cpu` from `ps` is a decaying average over ~the last minute, not an instant sample, so it can exceed 100% (that caveat is in the UI tooltip).
...
- **[LaunchdView.swift](file:///Users/nico/Workspace/SecretSauce/native/Sources/SecretSauce/Views/LaunchdView.swift):** The controls reduce to exactly two, in one row: an **Autostart** switch (permanent — `launchctl enable`/`disable`, survives reboot) and an **Activate/Deactivate** button (this session only — Activate loads and starts, Deactivate unloads). Deactivate unloads rather than `stop`s, because `stop` is useless against a `KeepAlive` job. There is deliberately no always-visible explanation block: the full "permanent vs. right now" explanation lives in the ⓘ tooltip next to the switch, and the status cards below carry their own hover tooltips. Status cards live in a `LazyVGrid(.adaptive)` — a fixed-width `HStack` overflowed the detail column past four cards.
